  ...
  <div class="w-full py-24 bg-black">
    ...
    >
      <div class="text-center mb-16">
        <h2 class="text-5xl md:text-6xl font-satoshi font-[900] text-white mb-4">Ride with Friends</h2>
        <p class="text-xl text-white/90 font-general-sans max-w-2xl mx-auto">
          Compare your performance with your riding buddies and discover who's setting the pace
        </p>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <!-- Friends Leaderboard -->
        <div>
          <h3 class="text-2xl font-satoshi font-bold text-white mb-6">Your Riding Crew</h3>
          <div class="space-y-4">
            <div
            ...
            >
              <div class="flex items-center space-x-4">
                <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-satoshi font-bold">
                  {{ friend.initial }}
                </div>
                <div class="flex-1">
                  <h4 class="text-lg font-satoshi font-bold text-white">{{ friend.name }}</h4>
                  <p class="text-sm text-white/60">{{ friend.location }} • {{ friend.bike }}</p>
                </div>
                <div class="text-right">
                  <div class="text-lg font-satoshi font-bold text-white">{{ friend.bestTime }}</div>
              ...
              <div class="mt-4 flex items-center space-x-2">
                ...
                <div class="text-sm font-medium" :class="friend.comparison.startsWith('+') ? 'text-red-400' : 'text-green-400'">
                  {{ friend.comparison }}
                </div>
              </div>
            </div>
        ...
        <div class="bg-zinc-900 rounded-3xl h-[500px] p-8 relative overflow-hidden">
          ...
          <div class="relative z-10 h-full">
            <h3 class="text-2xl font-satoshi font-bold text-white mb-6">Performance Comparison</h3>
            ...
            <div class="mt-8 bg-zinc-800 rounded-xl p-4">
              <h4 class="text-lg font-satoshi font-bold text-white mb-2">Social Insight</h4>
...
const friendsData = ref([
  { name: 'Marcus Chen', initial: 'M', location: 'Bay Area', bike: 'R1M', bestTime: '1:41.2', comparison: '-1.6s' },
  { name: 'Sarah Williams', initial: 'S', location: 'LA', bike: 'Panigale V4', bestTime: '1:42.1', comparison: '-0.7s' },
  { name: 'Jake Morrison', initial: 'J', location: 'Sacramento', bike: 'CBR1000RR', bestTime: '1:43.5', comparison: '+0.7s' },
  { name: 'Emma Rodriguez', initial: 'E', location: 'San Diego', bike: 'ZX-10R', bestTime: '1:44.2', comparison: '+1.4s' }
])
...
const createObserver = () => {
  ...
  observer = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      if (entry.isIntersecting && entry.target === friendsComparisonSection.value) {
...
onMounted(() => {
  createObserver()
  if (friendsComparisonSection.value) observer.observe(friendsComparisonSection.value)
})
