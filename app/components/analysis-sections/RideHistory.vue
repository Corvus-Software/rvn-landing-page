    ...
    <div class="w-full py-24 bg-gray-900">
        ...
        >
            <div class="text-center mb-16">
                <h2 class="text-5xl md:text-6xl font-satoshi font-[900] text-white mb-4">Ride History</h2>
                <p class="text-xl text-white/90 font-general-sans max-w-2xl mx-auto">Analyze your past rides and track your progress</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1 space-y-4 max-h-[600px] overflow-y-auto">
                    <div
                            ...
                            :class="{ 'bg-zinc-700 ring-2 ring-blue-500': activeRide === index }"
                            @click="setActiveRide(index)"
                    >
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-satoshi font-bold text-white">{{ ride.name }}</h3>
                            <div class="text-sm text-white/60 font-general-sans">{{ ride.date }}</div>
                        </div>

                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <div class="text-white/60 font-general-sans">Distance</div>
                                <div class="font-satoshi font-bold text-white">{{ ride.distance }}</div>
                            </div>
                            <div>
                                <div class="text-white/60 font-general-sans">Duration</div>
                                <div class="font-satoshi font-bold text-white">{{ ride.duration }}</div>
                            </div>
                        </div>

                        <div class="mt-4 flex items-center space-x-2">
                            <div class="w-2 h-2 rounded-full" :class="getPerformanceColor(ride.performance)"></div>
                            <span class="text-sm text-white/60 font-general-sans capitalize">{{ ride.performance }} Performance</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-zinc-800 rounded-3xl h-[600px] p-8 relative overflow-hidden">
                        ...
                        <div class="relative z-10 h-full">
                            <div class="flex items-center justify-between mb-8">
                                <h3 class="text-2xl font-satoshi font-bold text-white">{{ currentRide.name }}</h3>
                                <div class="text-white/60 text-sm font-general-sans">{{ currentRide.date }}</div>
                            </div>

                            <!-- Route Map Placeholder -->
                            <div class="bg-zinc-700 rounded-2xl h-64 mb-6 flex items-center justify-center">
                                <div class="text-center text-white">
                                    ...
                                    <div class="text-sm text-white/70">{{ currentRide.route }}</div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 gap-6">
                                <div class="text-center">
                                    <div class="text-2xl font-satoshi font-[900] text-green-400">{{ currentRide.maxSpeed }}</div>
                                    <div class="text-sm text-white/60">Max Speed</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-2xl font-satoshi font-[900] text-blue-400">{{ currentRide.avgLean }}</div>
                                    <div class="text-sm text-white/60">Avg Lean</div>
                                </div>
                                <div class="text-center">
                                    <div class="text-2xl font-satoshi font-[900] text-purple-400">{{ currentRide.score }}</div>
